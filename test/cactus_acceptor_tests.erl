...
acceptor_processes_carry_listener_name_and_index_label_test_() ->
    {setup,
        fun() ->
            {ok, _} = cactus_listener:start_link(
                acceptor_test_labels, #{port => 0, num_acceptors => 3}
            ),
            acceptor_test_labels
        end,
        fun(Name) -> ok = cactus_listener:stop(Name) end, fun(Name) ->
            {"acceptor pool labels include listener name + 1-based index", fun() ->
                ...
                AcceptorLabels = lists:sort([
                    L
                 || L <- Labels, is_tuple(L), element(1, L) =:= cactus_acceptor
                ]),
                ?assertEqual(
                    [
                        {cactus_acceptor, Name, 1},
                        {cactus_acceptor, Name, 2},
                        {cactus_acceptor, Name, 3}
                    ],
                    AcceptorLabels
                )
            end}
...
conn_process_carries_listener_name_and_peer_label_test_() ->
    {setup,
        fun() ->
            {ok, _} = cactus_listener:start_link(conn_test_labels, #{port => 0}),
            {conn_test_labels, cactus_listener:port(conn_test_labels)}
        end,
        fun({Name, _}) -> ok = cactus_listener:stop(Name) end, fun({Name, Port}) ->
            {"conn label is {cactus_conn, ListenerName, Peer} once peername is known", fun() ->
                ...
                ConnLabels = matching_labels(cactus_conn, Name),
                ?assertMatch([{cactus_conn, Name, {{127, 0, 0, 1}, _}}], ConnLabels),
                gen_tcp:close(Sock)
            end}
        end}.
...
%% Poll until we see a refined `{cactus_conn, Name, Peer}` label or run
...
wait_for_conn_label(Name, Sleep, Attempts) ->
    case matching_labels(cactus_conn, Name) of
        [{cactus_conn, Name, {_, _}}] ->
            ok;
        _ ->
            timer:sleep(Sleep),
