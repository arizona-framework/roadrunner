-module(cactus_uri_props).
-moduledoc """
Property-based tests for `cactus_uri`.
...
prop_percent_roundtrip() ->
    ?FORALL(
        Bin,
        binary(),
        {ok, Bin} =:= cactus_uri:percent_decode(cactus_uri:percent_encode(Bin))
    ).
...
prop_encode_output_is_unreserved_or_percent() ->
    ?FORALL(
        Bin,
        binary(),
        is_safe_encoded(cactus_uri:percent_encode(Bin))
    ).
