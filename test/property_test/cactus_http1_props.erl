-module(cactus_http1_props).
-moduledoc """
Property-based tests for `cactus_http1`'s incremental parsers.
...
prop_parse_request_line_never_crashes() ->
    ?FORALL(B, binary(), is_documented_shape(cactus_http1:parse_request_line(B))).

prop_parse_header_never_crashes() ->
    ?FORALL(B, binary(), is_documented_shape(cactus_http1:parse_header(B))).

prop_parse_headers_never_crashes() ->
    ?FORALL(B, binary(), is_documented_shape(cactus_http1:parse_headers(B))).

prop_parse_request_never_crashes() ->
    ?FORALL(B, binary(), is_documented_shape(cactus_http1:parse_request(B))).

prop_parse_chunk_never_crashes() ->
    ?FORALL(B, binary(), is_documented_shape(cactus_http1:parse_chunk(B))).
...
prop_parse_request_line_incremental() ->
    ?FORALL(
        Bytes,
        request_line_bytes(),
        cactus_http1:parse_request_line(Bytes) =:= feed(parse_request_line, Bytes)
    ).
...
feed(Fn, <<C, Rest/binary>>, Buf) ->
    NewBuf = <<Buf/binary, C>>,
    case cactus_http1:Fn(NewBuf) of
