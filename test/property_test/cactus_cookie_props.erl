-module(cactus_cookie_props).
-moduledoc """
Property-based tests for `cactus_cookie:parse/1`.
...
prop_parse_returns_well_formed_pairs() ->
    ?FORALL(
        B,
        binary(),
        lists:all(fun is_well_formed_pair/1, cactus_cookie:parse(B))
    ).
