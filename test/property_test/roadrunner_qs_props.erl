-module(roadrunner_qs_props).
-moduledoc """
Property-based tests for `roadrunner_qs`.
...
below avoids producing it, and an explicit `roadrunner_qs_tests` eunit
...
prop_parse_encode_roundtrip() ->
    ?FORALL(
        Pairs,
        list(qs_pair()),
        roadrunner_qs:parse(roadrunner_qs:encode(Pairs)) =:= Pairs
    ).
