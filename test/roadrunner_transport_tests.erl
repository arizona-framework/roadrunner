-module(roadrunner_transport_tests).

-include_lib("eunit/include/eunit.hrl").

%% =============================================================================
%% TLS hardened defaults — see roadrunner_transport:default_tls_opts/0 and the
%% OTP ssl_hardening guide.
%% =============================================================================

default_tls_opts_pins_security_keys_test() ->
    {ok, _} = application:ensure_all_started(ssl),
    Opts = roadrunner_transport:default_tls_opts(),
    ?assertEqual(
...
default_tls_opts_ciphers_are_aead_pfs_only_test() ->
    {ok, _} = application:ensure_all_started(ssl),
    {ciphers, Ciphers} = lists:keyfind(ciphers, 1, roadrunner_transport:default_tls_opts()),
...
default_tls_opts_signature_algs_excludes_sha1_test() ->
    ...
    {signature_algs, Algs} = lists:keyfind(
        signature_algs, 1, roadrunner_transport:default_tls_opts()
    ),
    ?assertNotEqual([], Algs),
...
apply_tls_defaults_with_empty_user_opts_returns_all_defaults_test() ->
    {ok, _} = application:ensure_all_started(ssl),
    ?assertEqual(
        roadrunner_transport:default_tls_opts(), roadrunner_transport:apply_tls_defaults([])
    ).

apply_tls_defaults_user_opt_overrides_default_test() ->
    {ok, _} = application:ensure_all_started(ssl),
    Result = roadrunner_transport:apply_tls_defaults([{versions, ['tlsv1.3']}]),
...
apply_tls_defaults_preserves_user_cert_opts_test() ->
    ...
    Result = roadrunner_transport:apply_tls_defaults(UserOpts),
...
port_on_closed_ssl_socket_returns_error_test() ->
    {ok, _} = application:ensure_all_started(ssl),
    ServerOpts = roadrunner_test_certs:server_opts(),
    {ok, S} = roadrunner_transport:listen_tls(
        0, ServerOpts ++ [binary, {active, false}, {reuseaddr, true}]
    ),
    roadrunner_transport:close(S),
    ?assertMatch({error, _}, roadrunner_transport:port(S)).
...
fake_send_forwards_to_owner_test() ->
    Sock = {fake, self()},
    ok = roadrunner_transport:send(Sock, [~"hello ", ~"world"]),
    receive
        {roadrunner_fake_send, From, Data} ->
...
fake_recv_blocks_for_reply_test() ->
    ...
    Self = self(),
    spawn(fun() ->
        Result = roadrunner_transport:recv(Sock, 0, 100),
        Self ! {recv_returned, Result}
    end),
    receive
        {roadrunner_fake_recv, ConnPid, _Len, _Timeout} ->
            ConnPid ! {roadrunner_fake_recv_reply, {ok, ~"data"}}
...
fake_close_forwards_to_owner_test() ->
    Sock = {fake, self()},
    ok = roadrunner_transport:close(Sock),
    receive
        {roadrunner_fake_close, From} -> ?assertEqual(self(), From)
...
fake_setopts_forwards_to_owner_test() ->
    Sock = {fake, self()},
    ok = roadrunner_transport:setopts(Sock, [{active, once}]),
    receive
        {roadrunner_fake_setopts, ConnPid, Opts} ->
...
setopts_on_real_gen_tcp_socket_test() ->
    ...
    ?assertEqual(ok, roadrunner_transport:setopts(Sock, [{active, once}])),
...
messages_returns_per_transport_atom_triples_test() ->
    ?assertEqual({tcp, tcp_closed, tcp_error}, roadrunner_transport:messages({gen_tcp, dummy})),
    ?assertEqual({ssl, ssl_closed, ssl_error}, roadrunner_transport:messages({ssl, dummy})),
    ?assertEqual(
        {roadrunner_fake_data, roadrunner_fake_closed, roadrunner_fake_error},
        roadrunner_transport:messages({fake, self()})
    ).
...
setopts_on_real_ssl_socket_test() ->
    ...
    ClientOpts =
        [{verify, verify_none} | roadrunner_test_certs:client_opts()] ++
            [binary, {active, false}],
    ...
    try
        ?assertEqual(
            ok,
            roadrunner_transport:setopts({ssl, ServerSock}, [{active, once}])
        ),
...
%% =============================================================================
%% sendfile/4 — gen_tcp covered via roadrunner_static integration tests; the
...
sendfile_via_fake_forwards_slice_test() ->
    ...
    try
        ok = roadrunner_transport:sendfile({fake, self()}, Path, 2, 5),
        receive
            {roadrunner_fake_send, From, Data} ->
                ?assertEqual(self(), From),
...
sendfile_missing_file_returns_error_test() ->
    ?assertMatch(
        {error, _},
        roadrunner_transport:sendfile({fake, self()}, "/no/such/path/roadrunner_xx", 0, 1)
    ).

sendfile_via_ssl_streams_through_chunked_fallback_test() ->
    %% TLS hides the kernel sendfile path, so roadrunner_transport falls
    ...
    try
        ...
        {ok, ClientSock} = ssl:connect({127, 0, 0, 1}, Port, ClientOpts, 5000),
        try
            ok = roadrunner_transport:sendfile(
...
sendfile_via_closed_gen_tcp_returns_error_test() ->
    ...
    try
        ...
        ?assertMatch({error, _}, roadrunner_transport:sendfile({gen_tcp, Sock}, Path, 0, 5)),
        gen_tcp:close(LSock)
    after
        file:delete(Path)
    end.
...
sendfile_via_ssl_send_error_returns_error_test() ->
    ...
    %% client closes the SSL socket before `roadrunner_transport:sendfile`
    ...
    try
        ...
        ?assertMatch(
            {error, _},
            roadrunner_transport:sendfile({ssl, ClientSock}, Path, 0, 96 * 1024)
        ),
        ssl:close(LSock)
    after
        file:delete(Path)
...
tmp_path(Suffix) ->
    filename:join(
        "/tmp",
        "roadrunner_sendfile_test_" ++ atom_to_list(Suffix) ++ "_" ++
...
fake_conn_drives_handler_without_sockets_test() ->
    ...
    %% roadrunner_test_handler sets `Connection: close`, so the conn closes.
    receive
        {roadrunner_fake_close, ConnPid} -> ok
    after 1000 -> error(no_close)
    end.

fake_proto_opts(Handler) ->
    #{
        dispatch => {handler, Handler, undefined},
        middlewares => [],
