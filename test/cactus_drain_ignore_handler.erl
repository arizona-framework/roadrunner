-module(cactus_drain_ignore_handler).
-moduledoc """
Test fixture — `{loop, ...}` handler that pushes a "started" chunk
and then ignores **everything**, including `{cactus_drain, _}`. Used
to verify `cactus_listener:drain/2` falls back to `exit(Pid, shutdown)`
