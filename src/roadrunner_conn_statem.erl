-module(roadrunner_conn_statem).
-moduledoc """
HTTP/1.1 connection state machine — the per-connection process
spawned by `roadrunner_conn:start/2`. Replaces the hand-rolled
`proc_lib` recursion that lived in `roadrunner_conn` before Phase 7 of
the migration.

`callback_mode/0` is `[handle_event_function, state_enter]` — every
event funnels through `handle_event/4`, which lets the
`{roadrunner_drain, _}` info event match a single wildcard-state clause
without duplicating the handler in every named state.

## States

- `awaiting_shoot` — gates the lifecycle until the acceptor's
  `shoot` message arrives. `roadrunner_acceptor:handle_accepted/2`
...
- `reading_request` — runs in **active-once** mode
  (`roadrunner_transport:setopts(_, [{active, once}])`). Inbound bytes
  ...
  be served).
- `reading_body` — calls `roadrunner_conn:read_body/4` (auto mode) or
  installs a `roadrunner_conn:make_body_state/4` body state (manual
  mode), then transitions to `dispatching`. Body reads still use
  the synchronous `roadrunner_conn:make_recv/3` closure; the deadline
  ...
  **Why mixed (active for headers, sync for body)?** Hibernation
  ...
  handlers call `roadrunner_req:read_body/1,2` synchronously, which
...
- `dispatching` — runs the middleware/handler pipeline, dispatches
  ...
  fires `[roadrunner, request, start | stop | exception]` telemetry,
...
- `[roadrunner, listener, accept]` — fired in the `shoot` handler once
  the peer is known.
- `[roadrunner, listener, conn_close]` — fired from `terminate/3`
  paired with the accept's `StartMono`. Skipped when termination
  happens before `shoot` (no matching accept event was emitted).
- `[roadrunner, request, start | stop | exception]` — fired from the
  dispatching pipeline.
- `[roadrunner, response, send_failed]` — bubbles up via
  `roadrunner_telemetry:response_send/2` from each socket write.
- `[roadrunner, ws, upgrade | frame_in | frame_out]` — fired by
  `roadrunner_ws_session` once dispatching delegates to it.
""".
...
-record(data, {
    ...
    %% with the `[roadrunner, listener, conn_close]` event in `terminate/3`.
    ...
    req :: roadrunner_http1:request() | undefined,
    buffered = <<>> :: binary(),
    ...
    response :: roadrunner_handler:response() | undefined,
    ...
    %% `roadrunner_listener:drain/2` broadcasts `{roadrunner_drain, Deadline}`
...
shared `ProtoOpts`. Mirrors `roadrunner_conn:start/2` exactly so the
...
-spec init({roadrunner_transport:socket(), roadrunner_conn:proto_opts()}) ->
    {ok, awaiting_shoot, #data{}}.
init({Socket, ProtoOpts}) ->
    ListenerName = maps:get(listener_name, ProtoOpts, undefined),
    proc_lib:set_label({roadrunner_conn, ListenerName}),
...
handle_event(info, {roadrunner_drain, _Deadline}, reading_request, Data) ->
    {stop, normal, Data};
handle_event(info, {roadrunner_drain, _Deadline}, _State, Data) ->
        ...
        manual ->
            case roadrunner_conn:body_framing(Req) of
                {error, FramingReason} ->
                    ...
                    {stop, normal, Data};
                Framing ->
                    BodyState = roadrunner_conn:make_body_state(Framing, Buffered, Recv, MaxCL),
                    {next_state, dispatching, Data#data{
...
handle_event(info, Unexpected, State, Data) ->
    logger:debug(#{
        msg => "roadrunner_conn_statem dropping unexpected info",
        state => State,
        info => Unexpected
    }),
    {keep_state, Data}.

%% Mirror `roadrunner_conn:handle_and_send/4` exactly: bracket the
%% middleware/handler pipeline with `[roadrunner, request, start | stop |
...
run_pipeline(Socket, Handler, Req, ListenerMws, #data{} = Data) ->
    ...
    StartMono = roadrunner_telemetry:request_start(Metadata),
    try Pipeline(Req) of
        {Response, Req2} when is_map(Req2) ->
            _ = dispatch_response(Socket, Handler, Req2, Response),
            ok = roadrunner_telemetry:request_stop(StartMono, Metadata, #{
        ...
        Class:Reason:Stack ->
            ok = roadrunner_telemetry:request_exception(StartMono, Metadata, Class, Reason),
            logger:error(#{
...
-spec request_timeout(first | keep_alive, roadrunner_conn:proto_opts()) -> non_neg_integer().
...
-spec pipelined_leftover(roadrunner_http1:request(), #data{}, binary()) -> binary().
...
-spec reading_request_timeouts(non_neg_integer(), roadrunner_conn:proto_opts()) ->
...
-spec rate_check_interval(roadrunner_conn:proto_opts()) -> pos_integer().
...
%% `roadrunner_conn:rate_ok/3`. After a 1-second grace, require the
...
%% Mirrors `roadrunner_conn:maybe_send_request_timeout/2` — only the very
...
-spec maybe_send_request_timeout(roadrunner_transport:socket(), first | keep_alive) ->
    ok | {error, term()}.
maybe_send_request_timeout(Socket, first) -> roadrunner_conn:send_request_timeout(Socket);
...
terminate(_Reason, _State, #data{
    ...
    roadrunner_conn:release_slot(ProtoOpts),
    ok.
