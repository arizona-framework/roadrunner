...
auth(Req, Next) ->
    case cactus_req:header(~"authorization", Req) of
        undefined -> {cactus_resp:unauthorized(), Req};
        _ -> Next(Req)
    end.
...
-type next() :: fun((cactus_http1:request()) -> cactus_handler:result()).
-type middleware() ::
    module()
    | fun((cactus_http1:request(), next()) -> cactus_handler:result()).
-type middleware_list() :: [middleware()].

-callback call(Request :: cactus_http1:request(), Next :: next()) ->
    cactus_handler:result().
